...
import { agentTokenOk, isLocalRequest } from "@/lib/local-only";
 ...
 * login, so each is instead restricted to requests that arrive on the
 * loopback interface (see lib/local-only.ts and the -H 127.0.0.1 bind):
...
export async function middleware(request: NextRequest) {
  ...
  if (isAgentReachable(pathname, request.method)) {
    ...
    if (isLocalRequest(request.headers) && agentTokenOk(request.headers)) {
