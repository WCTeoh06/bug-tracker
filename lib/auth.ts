...
import { agentTokenOk, isLocalRequest } from "./local-only";
 ...
 * request must be loopback-local (lib/local-only.ts) and, if AGENT_TOKEN is
 * configured, present it. This never grants write access - only the routes
 * that call this helper are GETs.
 */
export async function currentSessionOrLocalAgent(headers: Headers): Promise<ReadAuth | null> {
  const session = await currentSession();
  if (session) return session;
  if (isLocalRequest(headers) && agentTokenOk(headers)) return { agent: true };
  return null;
}
